#include "JVRC1PDController.h"

using namespace std;

namespace{
	const char* JVRC1PDController_spec[] =
	{
		"implementation_id", "JVRC1PDController",
		"type_name",         "JVRC1PDController",
...
JVRC1PDController::JVRC1PDController(RTC::Manager* manager)
...
JVRC1PDController::~JVRC1PDController()
{
}

void JVRC1PDController::ReadGain(size_t numJoints,std::vector<double> &pgain,std::vector<double> &dgain)
{
	FILE *PDGain;
...
RTC::ReturnCode_t JVRC1PDController::onInitialize()
{
	// Set InPort buffers
...
RTC::ReturnCode_t JVRC1PDController::onActivated(RTC::UniqueId ec_id)
{
	if(m_angleIn.isNew()){
		m_angleIn.read();
	...
	for(size_t i=0;i<m_angle.data.length();i++){
		//q_old[i] = q_old_ref[i] = m_angle.data[i];
...
RTC::ReturnCode_t JVRC1PDController::onDeactivated(RTC::UniqueId ec_id)
{
	cout<<"Deactivated"<<endl;
	return RTC::RTC_OK;
}

RTC::ReturnCode_t JVRC1PDController::onExecute(RTC::UniqueId ec_id)
{
	if(m_angleIn.isNew()){
		m_angleIn.read();
	}
...
extern "C"
{
	DLL_EXPORT void JVRC1PDControllerInit(RTC::Manager* manager)
	{
		coil::Properties profile(JVRC1PDController_spec);
		manager->registerFactory(profile,
				RTC::Create<JVRC1PDController>,
				RTC::Delete<JVRC1PDController>);
	}
};
